...
> Hint: 
> 
> Look for typos and if you're having trouble, look at the reactive graph
]
...
When using tidyverse (or othey non-standard evaluation package functions) with user data, use `!!` to match variables outside of the data. For example:
...
In short - a safe way to convert a text input to a variable name in dplyr is `!!sym(input$variable)`.
